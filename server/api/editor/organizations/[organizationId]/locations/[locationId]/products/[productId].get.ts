...
export default defineHandler(async (event) => {
  ...
  if (!siteId || !locationId || !productId) {
    return jsonResponse({ error: 'Site ID, location ID and product ID are required' }, { status: 400 })
  }
  try {
    return jsonResponse(await loadDashboardProduct(event, siteId, locationId, productId))
  } catch (error) {
    rethrowHttpError(error)
    console.error('location_product_read_failed', { siteId, locationId, productId, error: error instanceof Error ? error.message : String(error) })
