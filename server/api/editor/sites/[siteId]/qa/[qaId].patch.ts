...
export default defineHandler(async (event) => {
  ...
  try {
    return jsonResponse(await updateQa(db, {
      organizationId: site.organization_id, siteId, locationId: null, pagePath: typeof (body as ApiRecord).page_path === 'string' ? String((body as ApiRecord).page_path) : null, }, qaId, Object.fromEntries(Object.entries(body).filter(([key]) => key !== 'page_path'))))
  } catch (error) {
