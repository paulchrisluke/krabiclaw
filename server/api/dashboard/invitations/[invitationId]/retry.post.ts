  ...
  try {
    ...
    for (const scope of invitation.scopes) {
      const access = await ensureWhatsAppRecipientAccess(env, db, {
        organizationId: organization.id, siteId: scope.site_id, locationId: scope.location_id, phone: invitation.phone, inviterUserId: session.user.id, })
    ...
    await sendWhatsAppAccessInvitation(env, db, {
      organizationId: organization.id, siteId: primaryScope.site_id, locationId: primaryScope.location_id, phone: invitation.phone, invitationId: resolvedInvitationId, })
