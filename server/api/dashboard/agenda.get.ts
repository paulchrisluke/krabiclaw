...
export default defineHandler(async (event) => {
  ...
  const payload = await listAgenda(db, organization.id, {
    from, to, siteId: stringQuery(query.siteId), locationId: stringQuery(query.locationId), kinds: requestedKinds, organizationSlug: organization.slug, principal: { env, userId, role: organization.role }, })
