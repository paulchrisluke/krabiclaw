...
const VALID_TYPES: WorkRequestType[] = ['content_update', 'menu_update', 'translation', 'seo', 'google_business', 'seasonal', 'photo_update', 'social_media', 'technical', 'other']
...
export default defineEventHandler(async (event) => {
  ...
  const source = body.source === 'chowbot' ? 'chowbot' : body.source === 'whatsapp' ? 'whatsapp' : 'dashboard'

  if (!type || !VALID_TYPES.includes(type)) {
    return jsonResponse({ error: `Invalid type. Must be one of: ${VALID_TYPES.join(', ')}` }, { status: 400 })
  }
  if (!title) return jsonResponse({ error: 'Title is required' }, { status: 400 })
  if (!VALID_PRIORITIES.includes(priority)) return jsonResponse({ error: 'Invalid priority' }, { status: 400 })
  ...
  await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, organization.id, restaurant?.id ?? null, type, title, description, priority, source, now, now).run()
