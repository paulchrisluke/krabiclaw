...
export default defineHandler(async (event) => {
  ...
  const [nearby, attention] = await Promise.all([
    listAgenda(db, organization.id, {
      ...
      from: '2000-01-01', to: shiftUtcDay(now, 1), kinds: ['thread'], threadState: 'needs_attention', limit: 5, ...common, }), ])
  const items = nearby.items.filter(item => item.dayKey === todayKeyForTimeZone(now, item.timeZone))
  const counts = {
    reservations: items.filter(item => item.kind === 'reservation').length, experienceBookings: items.filter(item => item.kind === 'experience_booking').length, threadsNeedingAttention: attention.items.length, posts: items.filter(item => item.kind === 'post').length, }
  return jsonResponse(finalizeRequestMetrics(event, 'dashboard-today', {
    items, attention: [...attention.items].sort((left, right) => right.startsAt.localeCompare(left.startsAt)), counts, availableKinds: nearby.availableKinds, sites: nearby.sites, locations: nearby.locations, resolvedAt: now.toISOString(), }))
})
