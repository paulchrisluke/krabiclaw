...
export default defineHandler(async (event) => {
  ...
  const { env, db, organization, userId, location } = await getDashboardLocationContext(event, locationId)
  const organizationId = organization.id
  await assertLocationAccess(db, {
    env,
    userId, role: organization.role, organizationId, siteId: location.site_id, locationId, })
