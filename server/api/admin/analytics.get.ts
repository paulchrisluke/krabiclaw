...
import { adminHeadersForEvent, authAdminApi, countPlatformUsers, listPlatformOrganizations, platformPermissionError, requirePlatformEventPermission } from '~/server/utils/platform-admin-users'
...
export default defineHandler(async (event) => {
  ...
  try {
    ...
    return jsonResponse({
      metrics: {
        users: totalUsers, organizations: organizations.length, sites: totalSites?.count ?? 0, posts: totalPosts?.count ?? 0, menus: totalMenus?.count ?? 0, locations: totalLocations?.count ?? 0
