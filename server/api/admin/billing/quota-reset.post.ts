...
import {
  applyQuotaAdjustment, assertQuotaOperatorSession, parseQuotaAdjustmentRequest, previewQuotaAdjustment, QuotaAdjustmentError, } from '~/server/utils/quota-adjustment'
...
export default defineHandler(async (event) => {
  ...
  try {
    ...
    const result = await applyQuotaAdjustment(
      db, env.BETTER_AUTH_SECRET, request.input, actor, request.expectedStateSha256, request.approvalToken, organizationLookup, )
