...
export default defineHandler(async (event) => {
  ...
  try {
    const input = parsePlatformBlogLifecycleInput(await readBody(event) as unknown, 'unpublish')
    const lifecycle = await updatePlatformBlogLifecycle(db, postId, input)
    schedulePlatformKnowledgeIndexRebuild(event, env, 'blog post unpublish')
    ...
    if (statusCode >= 500) console.error('Failed to unpublish blog post:', error)
