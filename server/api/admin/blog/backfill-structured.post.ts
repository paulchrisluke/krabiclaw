...
export default defineHandler(async (event) => {
  ...
  const body = await readRequiredBody<{ apply?: boolean; site_id?: string | null }>(event)
  const report = await backfillLegacyBlogStructuredBlocks(db, {
    apply: body?.apply === true, ...(Object.prototype.hasOwnProperty.call(body ?? {}, 'site_id') ? { siteId: body.site_id ?? null } : {}), })
