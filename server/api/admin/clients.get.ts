...
function normalizeOrganization(value: BetterAuthOrganization | null, organizationId: string): ResolvedOrganization {
  ...
  return {
    id: organizationId,
    name: requireNonEmptyString(value.name, `organization ${organizationId} name`),
    slug: requireNonEmptyString(value.slug, `organization ${organizationId} slug`),
    createdAt: betterAuthTimestampToIso(createdAt as BetterAuthTimestamp, `organization ${organizationId}.createdAt`),
  }
}
...
function normalizeMember(value: BetterAuthMember, organizationId: string, expectedRole: 'owner' | 'admin') {
  ...
  return {
    userId,
    createdAt: betterAuthTimestampToIso(value.createdAt as BetterAuthTimestamp, `${expectedRole} member.createdAt`),
...
async function primaryWorkspaceMember(
  ...
  for (const role of ['owner', 'admin'] as const) {
    const result = await organizationAdapter.listMembers({
      ...
      sortBy: 'createdAt',
      sortOrder: 'asc',
      filter: { field: 'role', operator: 'eq', value: role },
    })
...
export default defineEventHandler(async (event) => {
  ...
  const resolvedClients = await Promise.all((clients ?? []).map(async (client) => {
    ...
    const organization = normalizeOrganization(
      await organizationAdapter.findOrganizationById(organizationId),
      organizationId,
    )
    return {
      ...client,
      ...
      impersonation_user_id: await primaryWorkspaceMember(organizationAdapter, organizationId),
      _organization_created_at: organization.createdAt,
    }
  }))
