...
export default defineHandler(async (event) => {
  ...
  try {
    const domains = await createCustomDomainPair(env, db, {
      siteId, organizationId: site.organization_id, domain: requestedDomain, includeWww, actorId: session.user.id, actorType
    })
    ...
    for (const domain of domains) {
      await notifyDomainLifecycle(env, db, {
        organizationId: site.organization_id, siteId, domain: domain.domain, status: domain.status, title: `Domain added: ${domain.domain}`, message: `DNS configuration is required for ${domain.domain}. Follow the dashboard instructions to complete setup.`, dashboardUrl
      })
    }
    ...
    console.error('domains_create_failed', {
      siteId, organizationId: site.organization_id, userId: session.user.id, requestedDomain, error: normalizedError.message, stack: normalizedError.stack || null
    })
