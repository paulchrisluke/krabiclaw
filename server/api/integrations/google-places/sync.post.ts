...
export default defineEventHandler(async (event) => {
  ...
  const { siteId, locationId } = body
  if (!siteId || !locationId) return jsonResponse({ error: 'siteId and locationId are required' }, { status: 400 })
  ...
  const location = await db.prepare(`
    ...
    LIMIT 1
  `).bind(locationId, siteId, site.organization_id).first<{ id: string; google_place_id: string | null }>()
