...
const PLATFORM_KNOWLEDGE_MUTATION_TOOLS = new Set([
  'create_platform_blog_post', 'update_platform_blog_metadata', 'replace_platform_blog_content', 'publish_platform_blog_post', 'unpublish_platform_blog_post', 'delete_platform_blog_post', 'create_platform_doc', 'update_platform_doc', 'publish_platform_doc', 'unpublish_platform_doc', 'delete_platform_doc', 'append_content_block', 'replace_content_block', 'delete_content_block', ])
    ...
    if (request.method === 'initialize') {
      ...
      return mcpSuccess(request.id, {
        protocolVersion, capabilities: { tools: {}, resources: {}, prompts: {} }, serverInfo: { name: 'krabiclaw-platform-mcp', version: 'v1' }, _meta: catalogMeta(PLATFORM_PUBLIC_MCP_TOOLS), instructions: [
          'KrabiClaw platform admin MCP. This app does not expose tenant site tools — it is for internal platform content operations and read-only release data.', 'You are acting as a growth/SEO copilot for a human content writer, not an autonomous publisher. Your job is to help them prioritize, research, draft, edit, and publish platform blog posts and docs in service of growing krabiclaw.com traffic and sign-ups.', 'Early in a session, read the kc://docs/product-context resource (PRODUCT.md) for what KrabiClaw actually is, its surfaces, verticals, and business model — do not assume or invent product facts.', 'Derive voice and tone from existing published posts/docs (via list_platform_blog_posts, get_platform_blog_post, list_platform_docs, get_platform_doc) rather than inventing a style — KrabiClaw has no separate style guide; the published content is the style guide.', 'Ground prioritization in get_platform_analytics (traffic and new_signups), not guesses about what readers want.', 'Use get_recent_changes when the writer needs categorized merged-pull-request source data for release notes, social posts, or product updates. Treat it as source material for human-reviewed drafts; it does not publish or send anything.', 'Before drafting or materially rewriting a platform blog post, call resolve_platform_agent_guidance({ task: "blog.write" }) and review_platform_agent_guidance_candidate({ task: "blog.write", candidate_type: "blog_draft", candidate: <exact draft> }). For tenant-targeted platform support, pass site_id so the scope is explicit.', 'For image-generation requests, call resolve_platform_agent_guidance({ task: "image.generate" }) and review_platform_agent_guidance_candidate({ task: "image.generate", candidate_type: "image_brief", candidate: <exact brief> }) before using native image_generation. Do not treat a generated image as stored platform media unless a platform media save tool exists for that flow.', 'If the writer wants to use an existing or attached featured/step image, use list_platform_media_assets to choose an existing asset or upload_platform_image with a real ChatGPT attachment.', 'Prefer the prompts audit_content_for_growth, draft_blog_post, and update_and_publish_post as starting points for those respective workflows.', 'New content should be drafted for the writer\'s approval before publishing. Once the writer has supplied or approved final content, execute the corresponding tool calls directly and in sequence — do not stop to describe a call instead of making it.', ].join(' '), })
    }
