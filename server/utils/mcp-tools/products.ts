...
const productListItemObject = {
  ...
  required: ['id', 'location_id', 'category', 'name', 'description', 'price', 'is_visible', 'available', 'sort_order'],
} as const
...
export const PRODUCTS_TOOLS: McpToolDefinition[] = [
  ...
  siteTool({ name: 'create_product', description: 'Create a Product at one explicit location.', domain: 'products', minimumRole: 'editor', confirmRequired: false, inputSchema: { location_id: { type: 'string' }, ...productWrite }, required: ['location_id', 'category', 'name', 'price'], outputSchema: productResult }),
  ...
  siteTool({ name: 'move_products', description: 'Use this when the user wants to reorder specific Products. Move only the named Products as one ordered block before another Product, or to the end when before_product_id is null. Never send the full catalog.', domain: 'products', minimumRole: 'editor', confirmRequired: false, inputSchema: { location_id: { type: 'string' }, product_ids: { type: 'array', minItems: 1, uniqueItems: true, items: { type: 'string' } }, before_product_id: { type: ['string', 'null'] } }, required: ['location_id', 'product_ids', 'before_product_id'], outputSchema: { type: 'object', properties: { moved: { type: 'boolean' } }, required: ['moved'] } }),
  siteTool({ name: 'move_product_category', description: 'Use this when the user wants to reorder an entire Product category or menu section. Move the named category before another category, or to the end when before_category is null. The server moves every Product in the category. Do not list or send Product IDs.', domain: 'products', minimumRole: 'editor', confirmRequired: false, inputSchema: { location_id: { type: 'string' }, category: { type: 'string' }, before_category: { type: ['string', 'null'] } }, required: ['location_id', 'category', 'before_category'], outputSchema: { type: 'object', properties: { moved: { type: 'boolean' } }, required: ['moved'] } }),
  siteTool({ name: 'rename_product_category', description: 'Atomically rename a Product category at one explicit location.', domain: 'products', minimumRole: 'editor', confirmRequired: false, inputSchema: { location_id: { type: 'string' }, old_category: { type: 'string' }, new_category: { type: 'string' } }, required: ['location_id', 'old_category', 'new_category'], outputSchema: { type: 'object', properties: { updated: { type: 'number' } }, required: ['updated'] } }),
  siteTool({ name: 'delete_product_category', description: 'Delete a Product category and its Products at one explicit location.', domain: 'products', minimumRole: 'editor', confirmRequired: true, inputSchema: { location_id: { type: 'string' }, category: { type: 'string' } }, required: ['location_id', 'category'], outputSchema: { type: 'object', properties: { deleted: { type: 'number' } }, required: ['deleted'] } }),
