...
export const PRODUCTS_TOOLS: McpToolDefinition[] = [
  siteTool({ name: 'list_products', description: 'List the products this site carries, published or withheld. The catalog belongs to the organization; a site carries a product through an explicit publication row.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { published_only: { type: 'boolean' }, ...paginationInputSchema }, required: [], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productListItemObject }, page_info: pageInfoObject }, required: ['products', 'page_info'] } }),
  ...
  siteTool({ name: 'get_product', description: 'Get a product with its options, variants, prices, publication, locations, collection membership and metafields.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { product_id: { type: 'string' } }, required: ['product_id'], outputSchema: productResult }),
  siteTool({ name: 'create_product', description: 'Create a product in the organization catalog and have this site carry it. Carrying is not publishing: use set_product_publication to make it visible.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { ...productWrite }, required: ['name'], outputSchema: productResult }),
  siteTool({ name: 'update_product', description: 'Replace a product with the intended state. Options, variants and prices are supplied whole; keep a variant id to keep its bookings. Removing a variant that has live bookings is refused.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { product_id: { type: 'string' }, ...productWrite }, required: ['product_id'], outputSchema: productResult }),
  siteTool({ name: 'delete_product', description: 'Delete a product and everything it owns. Refused while a canonical product page still points at it.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { product_id: { type: 'string' } }, required: ['product_id'], outputSchema: { type: 'object', properties: { deleted: { type: 'boolean' } }, required: ['deleted'] } }),
  siteTool({ name: 'set_product_publication', description: 'Publish or withhold a product on this site. Independent of the merchant sale switch and of location publication.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { product_id: { type: 'string' }, published: { type: 'boolean' } }, required: ['product_id', 'published'], outputSchema: productResult }),
  siteTool({ name: 'set_product_location', description: 'Say whether one location offers a product, and whether it shows on that location surfaces. A price does not establish where a product is sold.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { product_id: { type: 'string' }, location_id: { type: 'string' }, active: { type: 'boolean' }, published: { type: 'boolean' } }, required: ['product_id', 'location_id'], outputSchema: productResult }),
  siteTool({ name: 'remove_product_location', description: 'Stop offering a product at one location. The product and its other locations are untouched.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { product_id: { type: 'string' }, location_id: { type: 'string' } }, required: ['product_id', 'location_id'], outputSchema: productResult }),
  siteTool({ name: 'batch_create_products', description: 'Validate every row, then create all products atomically. Any invalid row rolls back the complete request.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { products: { type: 'array', minItems: 1, maxItems: PRODUCT_LIMITS.batchCreate, items: { type: 'object', properties: productWrite, required: ['name'], additionalProperties: false } } }, required: ['products'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productObject } }, required: ['products'] } }),
  siteTool({ name: 'reconcile_products', description: 'Create and update the supplied products in one idempotent pass, keyed by product_id. Products omitted from the request are left alone unless deactivate_missing is true, which turns off their sale switch — it never deletes them and never claims they are sold out.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { products: { type: 'array', maxItems: PRODUCT_LIMITS.reconcile, items: { type: 'object', properties: { product_id: { type: 'string', minLength: 1 }, ...productWrite }, required: ['name'], additionalProperties: false } }, deactivate_missing: { type: 'boolean' } }, required: ['products'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productObject } }, required: ['products'] } }),

  siteTool({ name: 'list_collections', description: 'List this site merchandising collections, in the order customers see them. Omit location_id for every collection; send it to narrow to one location.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { location_id: { type: ['string', 'null'] } }, required: [], outputSchema: { type: 'object', properties: { collections: { type: 'array', items: collectionObject } }, required: ['collections'] } }),
  ...
  siteTool({ name: 'update_collection', description: 'Rename a collection or change its description or position.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { collection_id: { type: 'string' }, name: { type: 'string' }, description: { type: ['string', 'null'] }, sort_order: { type: 'integer' } }, required: ['collection_id'], outputSchema: { type: 'object', properties: { collection: collectionObject }, required: ['collection'] } }),
  siteTool({ name: 'delete_collection', description: 'Delete a collection. The products in it are untouched.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { collection_id: { type: 'string' } }, required: ['collection_id'], outputSchema: { type: 'object', properties: { deleted: { type: 'boolean' } }, required: ['deleted'] } }),
  siteTool({ name: 'set_collection_products', description: 'Replace the membership and order of one collection with exactly the product ids you send, in that order. Anything you leave out is removed from this collection — the products themselves are untouched. Position lives on the membership, so the same product can sit third here and first elsewhere.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { collection_id: { type: 'string' }, product_ids: { type: 'array', items: { type: 'string' }, maxItems: PRODUCT_LIMITS.collectionProducts } }, required: ['collection_id', 'product_ids'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productListItemObject } }, required: ['products'] } }),
  siteTool({ name: 'reorder_collections', description: 'Change the order of whole collections. Send every collection id in the scope exactly once, in the intended order; a partial order is rejected.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { collection_ids: { type: 'array', items: { type: 'string' }, minItems: 1 }, location_id: { type: ['string', 'null'] } }, required: ['collection_ids'], outputSchema: { type: 'object', properties: { collections: { type: 'array', items: collectionObject } }, required: ['collections'] } }),

  siteTool({ name: 'list_metafield_definitions', description: 'List the tenant descriptive product attribute vocabulary. Adding an attribute of a supported type is one definition here, not a schema change.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: {}, required: [], outputSchema: { type: 'object', properties: { definitions: { type: 'array', items: metafieldDefinitionObject } }, required: ['definitions'] } }),
  ...
  siteTool({ name: 'delete_metafield_definition', description: 'Remove an attribute from the vocabulary. Its value is removed from every product that carried it.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { definition_id: { type: 'string' } }, required: ['definition_id'], outputSchema: { type: 'object', properties: { deleted: { type: 'boolean' } }, required: ['deleted'] } }),
]
