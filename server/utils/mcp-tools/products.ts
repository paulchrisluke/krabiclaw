...
export const PRODUCTS_TOOLS: McpToolDefinition[] = [
  ...
  siteTool({ name: 'set_collection_products', description: 'Set the complete membership and order of one collection. Send every product id in the intended order; a partial list is rejected. Position lives on the membership, so the same product can sit third here and first elsewhere.', domain: 'products', minimumRole: 'editor', confirmRequired: false, inputSchema: { collection_id: { type: 'string' }, product_ids: { type: 'array', items: { type: 'string' }, maxItems: PRODUCT_LIMITS.collectionProducts } }, required: ['collection_id', 'product_ids'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productListItemObject } }, required: ['products'] } }),
