...
export const PRODUCTS_TOOLS: McpToolDefinition[] = [
  ...
  organizationTool({ name: 'create_product', description: 'Create a product in the organization catalog and have this organization carry it. Carrying is not publishing: use set_product_publication to make it visible.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { ...productWrite }, required: ['name'], outputSchema: productResult }),
  ...
  organizationTool({ name: 'delete_product', description: 'Delete a product and everything it owns. Refused while a canonical product page still points at it.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { product_id: { type: 'string' } }, required: ['product_id'], outputSchema: { type: 'object', properties: { deleted: { type: 'boolean' } }, required: ['deleted'] } }),
  organizationTool({ name: 'set_product_publication', description: 'Publish or withhold a product on this organization. Independent of the merchant sale switch and of location publication.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { product_id: { type: 'string' }, published: { type: 'boolean' } }, required: ['product_id', 'published'], outputSchema: productResult }),
  organizationTool({ name: 'set_product_location', description: 'Say whether one location offers a product, and whether it shows on that location surfaces. A price does not establish where a product is sold.', domain: 'products', minimumRole: 'admin', confirmRequired: false, inputSchema: { product_id: { type: 'string' }, location_id: { type: 'string' }, active: { type: 'boolean' }, published: { type: 'boolean' } }, required: ['product_id', 'location_id'], outputSchema: productResult }),
  ...
  organizationTool({ name: 'set_collection_products', description: 'Replace the membership and order of one collection with exactly the product ids you send, in that order. Anything you leave out is removed from this collection — the products themselves are untouched. Position lives on the membership, so the same product can sit third here and first elsewhere.', domain: 'products', minimumRole: 'admin', confirmRequired: true, inputSchema: { collection_id: { type: 'string' }, product_ids: { type: 'array', items: { type: 'string' }, maxItems: PRODUCT_LIMITS.collectionProducts } }, required: ['collection_id', 'product_ids'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productListItemObject } }, required: ['products'] } }),
