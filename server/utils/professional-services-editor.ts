...
export async function upsertProfessionalServiceContent(
  ...
  for (const item of recordArray(data.offerings)) {
    ...
    await execute(db, `
      ...
      ON CONFLICT(id) DO UPDATE SET
        location_id = excluded.location_id, name = excluded.name, slug = excluded.slug, label = excluded.label,
  ...
  for (const item of recordArray(data.tenantPages)) {
    ...
    await execute(db, `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?)
      ON CONFLICT(id) DO UPDATE SET
        path = excluded.path, title = excluded.title, slug = excluded.slug, page_type = excluded.page_type,
  ...
  if (typeof data.themeTokens === 'object' && data.themeTokens) {
    await execute(db, `
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        updated_by = excluded.updated_by
    `, [`theme_${siteId}_blawby`, organizationId, siteId, json(data.themeTokens), updatedBy])
