...
export async function loadDashboardAdminBlogPost(event: H3Event, postId: string) {
  ...
  if (!session?.user?.email) throw createError({ statusCode: 401, statusMessage: 'Authentication required' })
  try {
    await requirePlatformEventPermission(event, env, { platform: ['content'] })
  } catch (error) {
    const permission = platformPermissionError(error)
    throw createError({ statusCode: permission.statusCode, statusMessage: permission.message })
  }
