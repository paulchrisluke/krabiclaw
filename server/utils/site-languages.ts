...
export async function addSiteLanguage(
  ...
  await execute(db, `
    ...
    VALUES (?, ?, ?, ?, ?, 0, 'disabled', ?, ?)
    ON CONFLICT(organization_id, organization_id, locale) DO UPDATE SET label = excluded.label, updated_at = excluded.updated_at
  `, [`locale::${input.organizationId}::${input.organizationId}::${locale}`, input.organizationId, input.organizationId, locale, catalog.label, now, now])
  return await loadLanguage(db, input.organizationId, input.organizationId, locale)
}
...
export async function publishSiteLanguage(
  ...
  const existing = await loadLanguage(db, input.organizationId, input.organizationId, locale)
  ...
  `, [now, now, input.organizationId, locale, input.organizationId, input.organizationId, locale])
...
export async function deleteDisabledSiteLanguageContent(
  ...
  const language = await loadLanguage(db, input.organizationId, input.organizationId, locale)
  ...
  const statements = [
    { query: `UPDATE organization_locales SET locale = NULL WHERE organization_id = ? AND organization_id = ? AND locale = ? AND status <> 'disabled'`, params: [input.organizationId, input.organizationId, locale] },
    { query: `DELETE FROM organization_redirects WHERE organization_id = ? AND organization_id = ? AND locale = ?`, params: [input.organizationId, input.organizationId, locale] },
    ...documents.flatMap(document => prepareContentDocumentDeletion({ documentId: document.id, organizationId: input.organizationId})),
    { query: `DELETE FROM resource_localizations WHERE organization_id = ? AND organization_id = ? AND locale = ?`, params: [input.organizationId, input.organizationId, locale] },
    { query: `DELETE FROM organization_locales WHERE organization_id = ? AND organization_id = ? AND locale = ? AND is_source = 0 AND status = 'disabled'`, params: [input.organizationId, input.organizationId, locale] },
  ]
