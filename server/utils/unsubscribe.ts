...
/**
 * The link an email footer carries. Null where there is nothing to unsubscribe
 * from: account security email is not optional, and an environment without the
 * signing secret cannot mint a link that would verify.
 */
export async function buildUnsubscribeUrl(
  ...
  const query = new URLSearchParams({ user: target.userId, category: target.category, token })
  return `https://${getPlatformDomain(env)}/unsubscribe?${query.toString()}`
}
