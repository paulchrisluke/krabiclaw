...
export const MCP_TOOLS: McpToolDefinition[] = [
  ...
  siteTool({
    ...
    description: 'Assign a saved media asset as the homepage hero image. Call get_site_media_assets first to find an active image asset id, then pass it here as asset_id. Use this immediately after upload_user_photo when the user wants their uploaded image on the homepage hero.',
    domain: 'content',
    minimumRole: 'editor',
    confirmRequired: false,
    inputSchema: {
  ...
  siteTool({
    ...
    description: `Create an experience. status must be one of: ${EXPERIENCE_STATUSES.join(', ')}. Every experience must belong to a location. Pass location_id directly, or omit it only when the site already has a primary location. If no location exists yet, call list_locations or create_location first.`,
    domain: 'experiences',
    minimumRole: 'editor',
    confirmRequired: false,
