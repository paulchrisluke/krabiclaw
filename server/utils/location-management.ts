...
async function validateMediaAsset(
  ...
  const asset = await db.prepare(`
    ...
    LIMIT 1
  `).bind(assetId, organizationId, siteId, kind).first()
...
async function loadLocation(
  ...
  return db.prepare(`
    ...
    LIMIT 1
  `).bind(locationId, organizationId, siteId).first<LocationRecord>()
}
...
export async function createLocation(
  ...
  if (input.rating !== undefined && input.rating !== null && (input.rating < 0 || input.rating > 5)) {
    return { status: 400, data: { error: 'rating must be between 0 and 5.' } }
  }
  if (input.review_count !== undefined && input.review_count !== null && input.review_count < 0) {
    return { status: 400, data: { error: 'review_count must be a whole number greater than or equal to 0.' } }
  }

  try {
    await validateMediaAsset(db, organizationId, siteId, input.hero_image_asset_id, 'image', 'hero_image_asset_id')
    await validateMediaAsset(db, organizationId, siteId, input.hero_video_asset_id, 'video', 'hero_video_asset_id')
  } catch (error) {
    return { status: 400, data: { error: error instanceof Error ? error.message : 'Invalid media asset.' } }
  }
  ...
  `).bind(organizationId, siteId).first<{ count: number | string }>()

  const activeCount = Number(activeCountRow?.count ?? 0)
  if (!Number.isFinite(activeCount)) {
    return { status: 500, data: { error: 'Unable to verify active locations.' } }
  }
  ...
  for (let attempt = 0; attempt < MAX_SLUG_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      void updateSubscriptionQuantity(env, db, organizationId).catch((error) => {
        console.error('Failed to update Stripe subscription quantity after location create:', error)
      })
  ...
  return {
    status: 409,
    data: { error: `Unable to allocate a unique location slug after ${MAX_SLUG_ATTEMPTS} attempts.` },
  }
}
...
export async function updateLocation(
  ...
  if (input.grab_url !== undefined) {
    try {
      ...
      return { status: 400, data: { error: error instanceof Error ? error.message : 'Invalid grab_url.' } }
    }
  }
  if (input.uber_eats_url !== undefined) {
    try {
      ...
      return { status: 400, data: { error: error instanceof Error ? error.message : 'Invalid uber_eats_url.' } }
    }
  }
  if (input.foodpanda_url !== undefined) {
    try {
      ...
      return { status: 400, data: { error: error instanceof Error ? error.message : 'Invalid foodpanda_url.' } }
    }
  }
...
export async function deleteLocation(
  ...
  const result = await db.prepare(`
    ...
    WHERE id = ? AND organization_id = ? AND site_id = ?
  `).bind(locationId, organizationId, siteId).run()
  ...
  await db.prepare(`
    ...
    SET primary_location_id = NULL, updated_at = ?, updated_by = ?
    WHERE id = ? AND organization_id = ? AND primary_location_id = ?
  `).bind(new Date().toISOString(), userId, siteId, organizationId, locationId).run()

  void updateSubscriptionQuantity(env, db, organizationId).catch((error) => {
    console.error('Failed to update Stripe subscription quantity after location delete:', error)
  })

  return { status: 200, data: { success: true, location_id: locationId, deleted: true } }
}
