...
export const merchant_handoff_destinations = sqliteTable("merchant_handoff_destinations", {
	id: text().primaryKey(),
	organization_id: text().notNull().references(() => organization.id, { onDelete: "cascade" } ),
	site_id: text().notNull().references(() => sites.id, { onDelete: "cascade" } ),
	...
	provider_location_id: text().notNull(),
	capabilities_json: text().notNull(),
	created_by: text().references(() => user.id, { onDelete: "set null" } ),
	created_at: text().default(sql`(strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))`).notNull(),
	updated_at: text().default(sql`(strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))`).notNull(),
}, (table) => [
	unique("merchant_handoff_destinations_scope_id_unique").on(table.id, table.organization_id, table.site_id, table.location_id),
	unique("merchant_handoff_destinations_id_version_unique").on(table.id, table.version),
	unique("merchant_handoff_destinations_scope_version_unique").on(table.organization_id, table.site_id, table.location_id, table.version),
	uniqueIndex("merchant_handoff_destinations_one_active_location_unique").on(table.organization_id, table.site_id, table.location_id).where(sql`status = 'active'`),
	index("merchant_handoff_destinations_oauth_client_idx").on(table.oauth_client_id),
	foreignKey({
		...
		name: "merchant_handoff_destinations_location_scope_fk",
	}),
	check("merchant_handoff_destinations_version_check", sql`${table.version} > 0`),
	check("merchant_handoff_destinations_status_check", sql`${table.status} IN ('active', 'inactive')`),
	check("merchant_handoff_destinations_endpoint_url_check", sql`${table.endpoint_url} LIKE 'https://_%' OR ${table.endpoint_url} LIKE 'http://localhost:%' OR ${table.endpoint_url} LIKE 'http://127.0.0.1:%'`),
	check("merchant_handoff_destinations_provider_check", sql`length(trim(${table.provider})) > 0`),
	check("merchant_handoff_destinations_provider_location_check", sql`length(trim(${table.provider_location_id})) > 0`),
]);

export const merchant_handoff_orders = sqliteTable("merchant_handoff_orders", {
	id: text().primaryKey(),
	organization_id: text().notNull().references(() => organization.id, { onDelete: "cascade" } ),
	site_id: text().notNull().references(() => sites.id, { onDelete: "cascade" } ),
	location_id: text().notNull().references(() => business_locations.id, { onDelete: "cascade" } ),
	destination_id: text().notNull().references(() => merchant_handoff_destinations.id, { onDelete: "restrict" } ),
	destination_version: integer().notNull(),
	order_version: integer().notNull(),
	provider_order_id: text(),
	provider_mappings_json: text().notNull(),
	order_snapshot_json: text().notNull(),
	merchant_state: text().default("pending").notNull(),
	fulfillment_state: text().default("unstarted").notNull(),
	...
	unique("merchant_handoff_orders_scope_id_unique").on(table.id, table.organization_id, table.site_id, table.location_id),
	unique("merchant_handoff_orders_id_version_unique").on(table.id, table.order_version),
	index("merchant_handoff_orders_location_created_idx").on(table.location_id, table.created_at),
	foreignKey({
		columns: [table.destination_id, table.organization_id, table.site_id, table.location_id],
		foreignColumns: [merchant_handoff_destinations.id, merchant_handoff_destinations.organization_id, merchant_handoff_destinations.site_id, merchant_handoff_destinations.location_id],
		name: "merchant_handoff_orders_destination_scope_fk",
	}),
	foreignKey({
		columns: [table.destination_id, table.destination_version],
		foreignColumns: [merchant_handoff_destinations.id, merchant_handoff_destinations.version],
		name: "merchant_handoff_orders_destination_version_fk",
	}),
	check("merchant_handoff_orders_destination_version_check", sql`${table.destination_version} > 0`),
	check("merchant_handoff_orders_order_version_check", sql`${table.order_version} > 0`),
	check("merchant_handoff_orders_state_version_check", sql`${table.state_version} > 0`),
	check("merchant_handoff_orders_merchant_state_check", sql`${table.merchant_state} IN ('pending', 'accepted', 'denied', 'cancelled')`),
	check("merchant_handoff_orders_fulfillment_state_check", sql`${table.fulfillment_state} IN ('unstarted', 'preparing', 'ready', 'completed', 'cancelled')`),
]);

export const merchant_handoff_deliveries = sqliteTable("merchant_handoff_deliveries", {
	id: text().primaryKey(),
	organization_id: text().notNull().references(() => organization.id, { onDelete: "cascade" } ),
	site_id: text().notNull().references(() => sites.id, { onDelete: "cascade" } ),
	location_id: text().notNull().references(() => business_locations.id, { onDelete: "cascade" } ),
	destination_id: text().notNull().references(() => merchant_handoff_destinations.id, { onDelete: "restrict" } ),
	order_id: text().notNull().references(() => merchant_handoff_orders.id, { onDelete: "cascade" } ),
	event_version: integer().default(1).notNull(),
	order_version: integer().notNull(),
	idempotency_key: text().notNull(),
	request_hash: text().notNull(),
	payload_snapshot_json: text().notNull(),
	...
	unique("merchant_handoff_deliveries_order_version_unique").on(table.order_id, table.order_version),
	unique("merchant_handoff_deliveries_order_key_unique").on(table.order_id, table.idempotency_key),
	index("merchant_handoff_deliveries_order_created_idx").on(table.order_id, table.created_at),
	foreignKey({
		columns: [table.order_id, table.organization_id, table.site_id, table.location_id],
		foreignColumns: [merchant_handoff_orders.id, merchant_handoff_orders.organization_id, merchant_handoff_orders.site_id, merchant_handoff_orders.location_id],
		name: "merchant_handoff_deliveries_order_scope_fk",
	}),
	foreignKey({
		columns: [table.order_id, table.order_version],
		foreignColumns: [merchant_handoff_orders.id, merchant_handoff_orders.order_version],
		name: "merchant_handoff_deliveries_order_version_fk",
	}),
	check("merchant_handoff_deliveries_event_version_check", sql`${table.event_version} > 0`),
	check("merchant_handoff_deliveries_order_version_check", sql`${table.order_version} > 0`),
	check("merchant_handoff_deliveries_status_check", sql`${table.status} IN ('pending', 'delivered', 'failed')`),
	check("merchant_handoff_deliveries_terminal_time_check", sql`(${table.status} = 'pending' AND ${table.delivered_at} IS NULL AND ${table.failed_at} IS NULL) OR (${table.status} = 'delivered' AND ${table.delivered_at} IS NOT NULL AND ${table.failed_at} IS NULL) OR (${table.status} = 'failed' AND ${table.delivered_at} IS NULL AND ${table.failed_at} IS NOT NULL)`),
]);

export const merchant_handoff_commands = sqliteTable("merchant_handoff_commands", {
	id: text().primaryKey(),
	organization_id: text().notNull().references(() => organization.id, { onDelete: "cascade" } ),
	site_id: text().notNull().references(() => sites.id, { onDelete: "cascade" } ),
	location_id: text().notNull().references(() => business_locations.id, { onDelete: "cascade" } ),
	destination_id: text().notNull().references(() => merchant_handoff_destinations.id, { onDelete: "restrict" } ),
	order_id: text().notNull().references(() => merchant_handoff_orders.id, { onDelete: "cascade" } ),
	command_version: integer().default(1).notNull(),
	order_version: integer().notNull(),
	expected_state_version: integer().notNull(),
	type: text().notNull(),
	idempotency_key: text().notNull(),
	request_hash: text().notNull(),
	provider_mappings_json: text().notNull(),
	command_snapshot_json: text().notNull(),
	status: text().default("pending").notNull(),
	...
	unique("merchant_handoff_commands_destination_key_unique").on(table.destination_id, table.idempotency_key),
	index("merchant_handoff_commands_order_created_idx").on(table.order_id, table.created_at),
	foreignKey({
		columns: [table.order_id, table.organization_id, table.site_id, table.location_id],
		foreignColumns: [merchant_handoff_orders.id, merchant_handoff_orders.organization_id, merchant_handoff_orders.site_id, merchant_handoff_orders.location_id],
		name: "merchant_handoff_commands_order_scope_fk",
	}),
	foreignKey({
		columns: [table.order_id, table.order_version],
		foreignColumns: [merchant_handoff_orders.id, merchant_handoff_orders.order_version],
		name: "merchant_handoff_commands_order_version_fk",
	}),
	check("merchant_handoff_commands_version_check", sql`${table.command_version} > 0`),
	check("merchant_handoff_commands_order_version_check", sql`${table.order_version} > 0`),
	check("merchant_handoff_commands_expected_state_version_check", sql`${table.expected_state_version} > 0`),
	check("merchant_handoff_commands_type_check", sql`${table.type} IN ('accept', 'deny', 'ready_time_update', 'ready', 'cancel', 'complete')`),
	check("merchant_handoff_commands_status_check", sql`${table.status} IN ('pending', 'applied', 'denied', 'error')`),
]);
