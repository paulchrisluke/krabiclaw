...
CREATE TABLE `__um_assert_0111` (
  `violation` text NOT NULL CHECK (`violation` = '')
);--> statement-breakpoint
INSERT INTO `__um_assert_0111` (`violation`)
SELECT 'business_locations_backup_count_mismatch'
WHERE (SELECT COUNT(*) FROM `__um_backup_business_locations`)
   != (SELECT COUNT(*) FROM `business_locations`);--> statement-breakpoint
INSERT INTO `__um_assert_0111` (`violation`)
SELECT 'pragma_foreign_key_check'
WHERE EXISTS (SELECT 1 FROM pragma_foreign_key_check);--> statement-breakpoint
DROP TABLE `__um_assert_0111`;--> statement-breakpoint
