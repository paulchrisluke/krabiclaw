...
CREATE TABLE `__new_sites` (
	...
	CONSTRAINT "sites_status_check" CHECK(status IN ('active', 'inactive', 'suspended')),
	CONSTRAINT "sites_plan_check" CHECK(plan IN ('free', 'growth', 'managed', 'seo_accelerator')),
	CONSTRAINT "sites_onboarding_status_check" CHECK(onboarding_status IN ('pending', 'active', 'failed')),
	CONSTRAINT "sites_url_structure_check" CHECK(url_structure IN ('location_subdirectories', 'brand_pages')),
	CONSTRAINT "sites_vertical_check" CHECK(vertical IN ('restaurant', 'experience', 'retail', 'wellness', 'service', 'professional_service')),
	CONSTRAINT "sites_content_source_check" CHECK(content_source IN ('google_maps', 'client_supplied', 'generated')),
	CONSTRAINT "sites_media_source_check" CHECK(media_source IN ('client_photos', 'stock', 'mixed'))
);
--> statement-breakpoint
INSERT INTO `__new_sites`("id", "organization_id", "theme_id", "theme", "slug", "subdomain", "custom_domain", "custom_domain_status", "primary_location_id", "public_url", "brand_name", "brand_description", "logo_url", "logo_asset_id", "contact_email", "contact_phone", "source_locale", "default_currency", "status", "plan", "onboarding_status", "url_structure", "vertical", "content_source", "media_source", "settings", "last_published_at", "created_at", "updated_at", "updated_by", "og_image_asset_id") SELECT "id", "organization_id", "theme_id", "theme", "slug", "subdomain", "custom_domain", "custom_domain_status", "primary_location_id", "public_url", "brand_name", "brand_description", "logo_url", "logo_asset_id", "contact_email", "contact_phone", "source_locale", "default_currency", "status", "plan", "onboarding_status", "url_structure", "vertical", "content_source", "media_source", "settings", "last_published_at", "created_at", "updated_at", "updated_by", "og_image_asset_id" FROM `sites`;--> statement-breakpoint
