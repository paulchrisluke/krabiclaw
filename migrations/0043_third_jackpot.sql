...
-- Recovery note: production was restored to its pre-0043 D1 bookmark after the generated
-- __new_sites rebuild cascaded through site-owned tables. D1 migration statements do not
-- preserve PRAGMA foreign_keys=OFF across the statement batches Wrangler sends, so dropping
-- this parent table deletes tenant data. The existing sites table already has the generated
-- CHECK constraints and UNIQUE(slug/subdomain) constraints; 0042 adds created_at. Only this
-- organization lookup index was genuinely missing, so add it without rebuilding the parent.
