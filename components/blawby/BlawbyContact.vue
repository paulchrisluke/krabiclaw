  ...
  <section class="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:px-8">
    <div>
      ...
      <form class="mt-10 grid gap-5" @submit.prevent="submitContact">
        ...
        <input id="contact-name" v-model="form.name" required type="text" placeholder="Name" class="border border-[var(--blawby-border)] px-4 py-3">
        <label for="contact-email" class="sr-only">Email</label>
        <input id="contact-email" v-model="form.email" required type="email" placeholder="Email" class="border border-[var(--blawby-border)] px-4 py-3">
        <label for="contact-subject" class="sr-only">Subject</label>
        <select id="contact-subject" v-model="form.subject" class="border border-[var(--blawby-border)] px-4 py-3">
          <option value="general">General question</option>
          <option value="partnerships">Partnerships</option>
          <option value="press">Press</option>
        </select>
        <label for="contact-message" class="sr-only">Message</label>
        <textarea id="contact-message" v-model="form.message" required rows="7" placeholder="Message" class="border border-[var(--blawby-border)] px-4 py-3" />
    ...
    <aside class="h-fit border border-[var(--blawby-border)] bg-white p-7">
      <h2 class="blawby-display text-3xl text-[var(--blawby-primary)]">Next step</h2>
