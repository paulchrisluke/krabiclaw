    ...
    <div class="mx-auto max-w-7xl px-0 py-4">
      <div class="border-b border-gray-200 bg-white pb-4">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div ref="menuRoot" class="relative z-10 inline-block text-left">
            <button
              type="button"
              class="group inline-flex items-center justify-center text-sm font-medium text-[var(--blawby-primary)] hover:text-[var(--blawby-primary-dark)]"
              :aria-expanded="open"
              aria-haspopup="menu"
              @click="open = !open"
            >
            ...
            <div v-if="open" class="absolute left-0 z-10 mt-1 w-72 origin-top-left rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5" role="menu">
      ...
      <div v-if="modelValue.length" class="bg-gray-100">
        <div class="mx-auto max-w-7xl px-4 py-3 sm:flex sm:items-center sm:px-6 lg:px-8">
          ...
          <div class="mt-2 flex flex-wrap items-center sm:ml-4 sm:mt-0">
            <span v-for="tag in modelValue" :key="tag" class="m-1 inline-flex items-center rounded-full border border-gray-200 bg-white py-1.5 pl-3 pr-2 text-sm font-medium text-[var(--blawby-primary-dark)]">
              {{ tag }}
              <button type="button" class="ml-1 inline-flex size-4 shrink-0 items-center justify-center rounded-full text-gray-400 hover:bg-gray-200 hover:text-gray-500" @click="remove(tag)">
