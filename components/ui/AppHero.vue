  ...
  <div class="relative w-full bg-black text-white overflow-hidden" :style="heroStyle">
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-4 flex flex-col items-center justify-center text-center h-full" :style="heroStyle">
      ...
      <div class="inline-flex items-center gap-2 mb-6">
        ...
        <span class="text-[10px] font-bold uppercase tracking-[0.3em] text-white/60 italic">
          {{ establishmentYearDisplay }}
        </span>
        <span class="w-8 h-px bg-white/30"></span>
      </div>
...
const establishmentYearDisplay = computed(() => {
  return props.establishmentYear ? `Est. ${props.establishmentYear}` : 'Est. Year'
})
