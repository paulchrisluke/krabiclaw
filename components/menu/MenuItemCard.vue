<template>
  <NuxtLink :to="`/menu/${item.slug}`" class="group flex gap-4 p-4 rounded-lg hover:bg-gray-50 transition-colors">
    <!-- Image -->
    ...
    <div class="flex-1 min-w-0">
      ...
      <div v-if="item.allergens?.length && !item.allergens.includes('PLACEHOLDER_ALLERGEN')" class="flex gap-1 mt-2 flex-wrap">
        <span
          ...
          :key="a"
          class="text-xs bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded"
        >
          {{ a }}
        </span>
      </div>
