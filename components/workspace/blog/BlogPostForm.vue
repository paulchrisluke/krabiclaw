  ...
  <div class="space-y-4">
    ...
    <UFormField label="Category">
      ...
      <UInput
        v-else
        v-model="form.category"
        placeholder="Enter category (e.g., Small Business, Special Education)"
      />
    </UFormField>
    ...
    <UFormField label="Featured Image">
      <slot name="media-picker" v-bind="{ modelValue: form.featured_image_asset_id, 'onUpdate:modelValue': (val: string) => form.featured_image_asset_id = val, onChange: handleImageChange }">
          ...
          <UCard v-for="(step, index) in form.how_to_steps" :key="`howto-${index}`">
            <div class="space-y-3">
              ...
              <UFormField label="Step image">
                <slot name="media-picker" v-bind="{ modelValue: step.image_asset_id, 'onUpdate:modelValue': (val: string) => step.image_asset_id = val }">
...
const form = defineModel<ReturnType<typeof useBlogForm>['form']>({ required: true })
...
const categoryItems = computed(() => {
  if (props.freeTextCategory) {
    return []
  }
  return BLOG_CATEGORY_LABELS.map((item) => ({ label: item, value: item }))
})
const robotsItems = [
...
function addFaqItem() {
  form.value.faq_items.push(createEmptyFaqItem())
}
...
function addHowToStep() {
  form.value.how_to_steps.push(createEmptyHowToStep())
}

function removeHowToStep(index: number) {
  form.value.how_to_steps.splice(index, 1)
}
...
defineExpose({ form, canSave, canPublish })
</script>
