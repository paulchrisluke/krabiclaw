...
// Shared by pages/experiences/[slug].vue (English, matched directly by
// Nuxt's file router) and the locale-prefixed catch-all
// (pages/[...tenantPath].vue, representation.kind === 'resource' with
// resource_type 'experience'). See components/tenant-pages/LocationQaPage.vue
// for why translated resource pages can't be a route alias onto the English
// page instead - this is the same split, applied so a translated experience
// gets the real themed page (gallery, booking, policy summary, structured
// data) instead of the generic title/summary/body/details fallback.
...
const slug = computed(() => experience.value?.slug ?? String(route.params.slug ?? ''))
...
async function loadAvailability() {
  ...
  try {
    ...
    const res = await $fetch<{ timezone: string; dates: RawDateAvailability[] }>(
      `/api/public/sites/${siteId}/experiences/${slug.value}/availability`,
...
async function submitBooking() {
  ...
  try {
    const res = await $fetch<{ success: boolean; message: string; booking_id: string; cancellation_token: string; policy_summary?: ApiRecord | null }>(
      `/api/public/sites/${siteId}/experiences/${slug.value}/book`,
      {
        method: 'POST',
        body: {
...
const resolvedCanonicalUrl = computed(() => canonicalUrl.value || `${siteUrl}/experiences/${slug.value}`)

useBreadcrumbSchema([
  { name: 'Home', url: `${siteUrl}/` },
  { name: 'Experiences', url: `${siteUrl}/experiences` },
  { name: experience.value?.title ?? slug.value, url: resolvedCanonicalUrl.value },
])
