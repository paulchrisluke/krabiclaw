...
// Shared by pages/blog/[slug].vue (English, matched directly by Nuxt's file
// router) and the locale-prefixed catch-all (pages/[...tenantPath].vue,
// representation.kind === 'resource' with resource_type 'tenant_blog_post').
// See components/tenant-pages/LocationQaPage.vue for why this can't be a
// route alias instead. Only metadata (title/excerpt/category/tags/nav_title/
// seo_*) is registered as localizable - content_blocks (the article body)
// has no translation mechanism, so it always renders in English.
...
const postEndpoint = computed(() => `/api/public/sites/${siteId}/blog/${slug.value}`)
...
const { data, pending, error } = await useAsyncData(
  () => `tenant-blog-post-${siteId}-${slug.value}-${locale.value}`,
  async () => {
    ...
    if (import.meta.server) {
      ...
      post = await getPublishedSiteBlogPost(db, siteId, slug.value, env, locale.value) as TenantBlogPost | null
    } else {
