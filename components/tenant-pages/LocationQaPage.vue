...
// Shared by pages/locations/[slug]/qa.vue (English, matched directly by Nuxt's
// file router) and the locale-prefixed catch-all (pages/[...tenantPath].vue,
// representation.kind === 'location_subpage') - one implementation, two entry
// routes, since a locale-prefixed path can't alias onto the English page file.
const { t } = useI18n()
...
// Falls back to the raw route param so this still works when Nuxt's router
// matches this content directly at /locations/[slug]/qa (location.value.slug
// resolves the same value there too, but only once usePublicPageData settles).
