<template>
  <UPage>
    <UPageHeader title="Reviews" description="Moderate reviews and owner replies across every location.">
      <template #links>
...
async function loadReviews() {
  ...
  try {
    ...
    const batches = await Promise.all(locations.value.map(async (location) => {
      const res = await $fetch<{ reviews: Omit<ReviewRow, 'locationTitle'>[] }>(`/api/sites/${siteId}/locations/${location.id}/reviews`)
...
async function saveReply() {
  ...
  saving.value = true
  try {
    await $fetch(`/api/editor/sites/${siteId}/reviews/${activeReview.value.id}`, {
      method: 'PATCH',
...
async function setStatus(review: ReviewRow, status: 'approved' | 'rejected') {
  try {
    await $fetch(`/api/editor/sites/${siteId}/reviews/${review.id}`, {
      method: 'PATCH',
      body: { status }
    })
...
async function saveReview() {
  ...
  try {
    ...
    if (editingReviewId.value) {
      await $fetch(`/api/sites/${siteId}/locations/${reviewForm.location_id}/reviews/${editingReviewId.value}`, {
        method: 'PATCH',
        body
      })
      toast.add({ description: 'Review updated', color: 'success' })
    } else {
      await $fetch(`/api/sites/${siteId}/locations/${reviewForm.location_id}/reviews`, {
        method: 'POST',
        body
      })
...
async function deleteReview(review: ReviewRow) {
  ...
  try {
    await $fetch(`/api/sites/${siteId}/locations/${review.location_id}/reviews/${review.id}`, { method: 'DELETE' })
