<template>
  <div class="min-h-screen bg-(--ui-bg)">
    ...
    <div class="max-w-6xl mx-auto px-4 py-12">
      <div class="mb-12">
        <h2 class="text-2xl md:text-3xl font-bold text-(--ui-text-highlighted) mb-6">Find Us</h2>
        <div class="aspect-video overflow-hidden rounded-2xl bg-(--ui-bg-muted)">
          <div class="flex h-full w-full items-center justify-center">
            <div class="px-8 text-center">
              <div class="mx-auto mb-4 flex size-14 items-center justify-center rounded-2xl bg-(--ui-bg) text-(--ui-text-highlighted) shadow-sm">
              ...
              <p class="mt-2 text-sm text-(--ui-text-muted)">Connect Google Business to sync verified directions.</p>
            </div>
          </div>
        </div>
        <div class="mt-4 text-center">
          <span class="inline-block cursor-not-allowed rounded-full border border-(--ui-border) px-6 py-3 font-semibold text-(--ui-text-dimmed)">Get Directions →</span>
        </div>
      </div>
      <div class="grid md:grid-cols-2 gap-12 mb-12">
        <div>
          <h2 class="text-2xl md:text-3xl font-bold text-(--ui-text-highlighted) mb-6">Contact Information</h2>
          <div class="space-y-6">
            <div>
              <h3 class="font-semibold text-(--ui-text-highlighted) mb-1 uppercase tracking-wider text-xs">Restaurant</h3>
              <p class="text-(--ui-text) text-lg">{{ getField('businessName', businessName) || 'Connect Google Business' }}</p>
            </div>
            <div>
              <h3 class="font-semibold text-(--ui-text-highlighted) mb-1 uppercase tracking-wider text-xs">Address</h3>
              <p class="text-(--ui-text) text-lg">
                {{ getField('address', businessAddress) || 'Connect Google Business' }}
                <span v-if="!getField('address', businessAddress)" class="text-sm text-(--ui-text-dimmed) block">Connect Google Business to sync the verified address.</span>
              </p>
            </div>
            <div>
              <h3 class="font-semibold text-(--ui-text-highlighted) mb-1 uppercase tracking-wider text-xs">Phone</h3>
              <p class="text-(--ui-text) text-lg">
                {{ getField('phoneNumber', businessPhone) || 'Connect Google Business' }}
                <span v-if="!getField('phoneNumber', businessPhone)" class="text-sm text-(--ui-text-dimmed) block">Connect Google Business to sync the verified phone number.</span>
              </p>
            </div>
          </div>
          <div v-if="parkingInfo" class="mt-8">
            <h3 class="font-semibold text-(--ui-text-highlighted) mb-2 uppercase tracking-wider text-xs">Parking</h3>
            <div v-html="parkingInfo" class="text-(--ui-text-muted) text-sm" />
          </div>
          <div v-if="extraNotes" class="mt-4">
            <h3 class="font-semibold text-(--ui-text-highlighted) mb-2 uppercase tracking-wider text-xs">Additional Notes</h3>
            <div v-html="extraNotes" class="text-(--ui-text-muted) text-sm" />
          </div>
        </div>
        <div>
          <h2 class="text-2xl md:text-3xl font-bold text-(--ui-text-highlighted) mb-6">Opening Hours</h2>
              ...
              <tbody>
                <tr v-for="hour in (businessHoursFormatted.length > 0 ? businessHoursFormatted : defaultHours)" :key="hour.day" class="border-b border-(--ui-border) last:border-0">
                  ...
                  <td class="py-3 text-right text-(--ui-text-highlighted)">
                    {{ hour.hours }}
                    <span v-if="businessHoursFormatted.length === 0" class="text-sm text-(--ui-text-dimmed) block">Connect Google Business to keep hours fresh.</span>
                  </td>
                </tr>
              </tbody>
            </table>
