<template>
  <div class="min-h-screen bg-white">
    <AppHero :title="getField('hero.title', `${location?.title || 'Location'} Menu`)" :subtitle="getField('hero.subtitle', 'Location-specific menu')" size="page" />
    ...
    <!-- Fallback notice -->
    <AppSection v-else-if="isUsingBrandMenu && hasMenu" bg="yellow" padding="default">
      <div class="text-center">
        <p class="text-yellow-800">Showing brand menu (location-specific menu not available)</p>
      </div>
    </AppSection>
    ...
    <AppSection v-else bg="white" padding="default">
      <div class="text-center py-12">
        <div class="mb-6">
          ...
          <p class="text-gray-600 mb-6">Our delicious menu for {{ location?.title || 'this location' }} is being prepared. Check back soon!</p>
        </div>
...
// Get location-specific menu (falls back to brand menu if no location menu)
const { 
  menu, 
  loading: menuLoading, 
  error: menuError, 
