...
const { getCurrentCreditGrantProjection, grantQuota } = await import('../../server/utils/usage-metering.ts')
...
test('quota access follows Better Auth and fails closed after trial expiry', async () => {
  const db = createDb()
  seedSubscription(db, {
