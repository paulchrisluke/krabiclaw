...
test('shared blog creation is draft-only and rejects lifecycle fields', async () => {
  ...
  assert.match(createFlow, /createContentDocumentWithBlocks\(db, ownerType, id, canonicalBlocks, \{[\s\S]*additionalQueriesBefore: \[blogPostInsert\]/)
  assert.doesNotMatch(createFlow, /revision|publish:/)
