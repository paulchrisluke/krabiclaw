...
test('one exact current-runtime event is accepted while ambiguous or mismatched history is rejected', async () => {
  ...
  assert.equal(exactPlan.backfillCount, 1)
  assert.equal(exactPlan.matchedCount, 1)
...
test('in-batch CAS rolls back on concurrent legacy log, canonical event, grant, or credit changes', async () => {
  ...
  for (const scenario of scenarios) {
    const db = createDb()
    seed(db, '2026-08-10')
    const plan = await previewHistoricalUsageReconciliation(db as never, SECRET, input({ idempotencyKey: `race-${scenario.name}` }), 'operator-1', NOW)
