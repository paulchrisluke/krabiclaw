...
test('full lane keeps one uninterrupted staging candidate lock and gates candidate promotion', async () => {
  ...
  assert.doesNotMatch(source, /stripe-catalog-plan|stripeCatalogPreflight|STRIPE_CATALOG_PREFLIGHT/)
  assert.match(source, /preflight-stripe-webhook-endpoint\.mjs/)
  assert.match(source, /stripe-testmode-canary\.spec\.ts/)
