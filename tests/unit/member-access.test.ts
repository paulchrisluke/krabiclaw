...
test('the audited /api/dashboard/** boundary is deny-by-default for scoped roles', () => {
  ...
  assert.equal(canScopedRoleUseDashboardPath('/api/dashboard/locations/loc-1/integrations/google-business/auth'), false)
  ...
  assert.equal(canScopedRoleUseDashboardPath('/api/dashboard/ai/generate-image'), false)
  assert.equal(canScopedRoleUseDashboardPath('/api/dashboard/ai/credits'), false)
