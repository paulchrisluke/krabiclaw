...
test('MCP annotation validation accepts only internally consistent hint combinations', () => {
  ...
  for (const name of ['update_product', 'set_collection_products', 'delete_collection', 'reconcile_products', 'reorder_media', 'update_media_asset', 'update_organization_settings']) {
