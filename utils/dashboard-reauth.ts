...
const MESSAGES_DEEP_LINK_PATH = /^\/dashboard\/[^/]+\/sites\/[^/]+(?:\/locations\/[^/]+)?\/messages(?:\/.*)?$/

/**
 * True when `path` is the WhatsApp-notification-driven guest-thread deep link
 * (`/dashboard/{org}/sites/{site}/messages[/threadId]` or
 * `/dashboard/{org}/sites/{site}/locations/{location}/messages[/threadId]`) — the dashboard
 ...
 */
export function isWhatsAppMessagesDeepLinkPath(path: string): boolean {
  return MESSAGES_DEEP_LINK_PATH.test(path)
}
