...
- On Saya's public component tree (`components/saya/**`), prefer plain `<button>`/`<NuxtLink>`/`<a>` + Tailwind classes over `UButton`, and inline SVG over `UIcon`, for anything in the always-rendered header/footer path. This is a _perf-driven exception_ to the Nuxt UI default, scoped to public/high-traffic Saya components — it does not apply to the dashboard, to Saya's own auth-gated dashboard-CMS edit affordances, or to low-traffic Saya pages that aren't part of every page load.
