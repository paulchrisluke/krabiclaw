...
**Known perf issue, not yet fixed (2026-07-02):** `app.vue`'s `loadGa4()` is deferred via `requestIdleCallback(loadGa4, { timeout: 8000 })` with a comment claiming this "protects Lighthouse TTI." It does the opposite — `requestIdleCallback` fires as soon as the main thread goes idle, which on a content-light page happens almost immediately, injecting a 166KB `gtag.js` fetch right into the window Lighthouse needs quiet to mark the page interactive. Reproduced via interleaved A/B (GA4 on vs `PERF_NO_GA4=true`, 8 rounds, real production builds via local `wrangler dev`, simulated mobile throttling): GA4-on TTI median 4.51s vs GA4-off 2.98s, GA4 slower in 8/8 rounds. This affects every platform page (and every tenant page with a connected GA4) equally — it is not specific to any one page or component. See `HANDOFF-page-speed-2026-07-02.md` for the full repro methodology and raw data before touching this.
