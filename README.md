...
fixture, credential, and foreign-key baseline. Then sign in with:

```text
Email: demo-owner@playwright.example
Password: krabiclaw-local-dev
```
...
`krabiclaw-local-dev` is a public, local-only fixture password. Better Auth
stores only its hash in local D1. Set `E2E_TEST_PASSWORD` before
`corepack yarn local:setup` to replace it. A repeated setup replaces fixture
passwords and deletes their existing sessions, so sign in again afterward.
