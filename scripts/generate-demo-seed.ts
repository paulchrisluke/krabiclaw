...
INSERT OR REPLACE INTO sites (id, organization_id, theme_id, theme, slug, subdomain, brand_name, public_url, status, plan, onboarding_status, default_currency, vertical, created_at, updated_at)
VALUES ('site-mcp-growth-service-selection', ${sqlValue(orgId)}, 'saya-theme-v1', 'saya', 'mcp-growth-service-selection', 'mcp-growth-service-selection', 'MCP Selection Fixture', 'https://mcp-growth-service-selection.krabiclaw.com', 'active', ${sqlValue(plan)}, 'active', 'THB', 'restaurant', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
INSERT OR REPLACE INTO sites (id, organization_id, theme_id, theme, slug, subdomain, brand_name, public_url, status, plan, onboarding_status, default_currency, vertical, created_at, updated_at)
VALUES (${sqlValue(siteId)}, ${sqlValue(orgId)}, 'saya-theme-v1', 'saya', ${sqlValue(slug)}, ${sqlValue(slug)}, ${sqlValue(name)}, ${sqlValue(`https://${slug}.krabiclaw.com`)}, 'active', ${sqlValue(plan)}, 'active', 'THB', 'restaurant', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
