...
assert(workflow.includes("needs.e2e-plan.outputs.run_full_lanes == 'true'"), 'High-impact pull requests must use the isolated full-suite lanes')
assert(!workflow.includes('\n  e2e-lane-smoke:'), 'Lane contract smoke must run inside each full-suite shard job')
assert(workflow.indexOf('run: yarn test:e2e:lane-smoke') < workflow.indexOf('run: yarn test:e2e:full --shard='), 'Each isolated lane must pass its contract smoke before its full-suite shard')
