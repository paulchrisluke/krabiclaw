...
const ALLOWED_VERTICALS = ['experience', 'restaurant', 'retail', 'wellness', 'service']
if (VERTICAL && !ALLOWED_VERTICALS.includes(VERTICAL)) {
  console.error(`Error: Invalid vertical "${VERTICAL}". Must be one of: ${ALLOWED_VERTICALS.join(', ')}`)
  process.exit(1)
}
...
if (!SLUG) {
  ...
  console.error('Usage: node scripts/client-import.mjs --slug <slug> [--dry-run | --approve | --apply] [--allow-stock] [--remote]')
  process.exit(1)
}
...
function _uid(prefix = '') {
  return `${prefix}${createHash('sha256').update(SLUG + Date.now() + Math.random()).digest('hex').slice(0, 12)}`
}
...
function checkRouteParity() {
  ...
  for (const ct of CONTENT_TYPES) {
    ...
    if (hasFlat && hasSlug) {
      issues.push({ type: 'ERROR', message: `pages/${ct}.vue conflicts with pages/${ct}/[slug].vue — move to pages/${ct}/index.vue` })
    }
    // Conflict: flat file + directory index = duplicate routes
    if (hasFlat && hasIndex) {
      issues.push({ type: 'ERROR', message: `pages/${ct}.vue and pages/${ct}/index.vue both exist — remove one` })
    }
    // Warn: has slug routes but only flat index (no directory index alongside slug)
    if (hasFlat && !hasIndex && hasSlug) {
      issues.push({ type: 'WARN', message: `pages/${ct}.vue exists alongside pages/${ct}/[slug].vue — consider moving to pages/${ct}/index.vue` })
    }
    ...
    if (hasIndex && !hasSlug && ['experiences', 'menu', 'posts'].includes(ct)) {
      issues.push({ type: 'WARN', message: `pages/${ct}/[slug].vue does not exist — detail pages will 404` })
    }
  }
...
async function resolvePlace(rawUrl) {
  ...
  if (!placeId) {
    return {
      source_url: rawUrl,
      name: name ?? '(unknown)',
      error: API_KEY ? 'Could not resolve place ID' : 'GOOGLE_MAPS_API_KEY not set — skipping Places API lookup',
    }
  }
...
function detectMissingFields(places) {
  ...
  for (const place of places) {
    if (place.error) {
      missing.push({ location: place.name, field: 'place_data', issue: place.error })
      continue
    }
    if (!place.phone && !place.international_phone) missing.push({ location: place.name, field: 'phone' })
    if (!place.opening_hours) missing.push({ location: place.name, field: 'opening_hours' })
    if (!place.lat || !place.lng) missing.push({ location: place.name, field: 'coordinates' })
    if (!place.formatted_address) missing.push({ location: place.name, field: 'address' })
    if (!place.reviews?.length) missing.push({ location: place.name, field: 'reviews', issue: 'No reviews returned from API' })
    if (!place.photos?.length) missing.push({ location: place.name, field: 'photos', issue: 'No photos returned from API (may need manual upload)' })
  }
...
const FORBIDDEN_BY_VERTICAL = {
  experience: [
    'restaurant', 'kitchen', 'dining', 'chef', 'menu', 'table reservation',
    'catering', 'dine', 'Come dine', 'From the kitchen', 'Reserve a table',
    'one kitchen philosophy', 'Dietary needs.*preferred seating',
  ],
  restaurant: [],
  retail: [],
  wellness: [],
...
function generateSeedSql(places, mediaManifest) {
  ...
  const locationInserts = places.map((place, idx) => {
...
function generateRouteManifest(places) {
  ...
  if (VERTICAL === 'experience') {
    manifest._note = 'Run client:verify --site-id to discover experience slugs via bootstrap API'
  }
  ...
  try {
    const result = spawnSync(
      ...
      ['wrangler', 'd1', 'execute', 'DB', flag, '--command',
       `SELECT COUNT(*) as n FROM ${table} WHERE site_id = '${siteId}'`, '--json'],
      { encoding: 'utf8', cwd: process.cwd() }
    )
    if (result.status !== 0) return null
    const jsonMatch = (result.stdout + (result.stderr ?? '')).match(/\[[\s\S]*?\]/)
...
if (MODE === 'approve') {
  ...
  if (!existsSync(manifestPath) || !existsSync(seedPath)) {
    console.error('Error: No dry-run output found. Run --dry-run first to generate manifests.')
    process.exit(1)
  }
  ...
  if (existsSync(mediaManifestPath)) {
    try {
      ...
      mediaMeta = { directory: mm.directory ?? null, dir_mtime: mm.dir_mtime ?? null, count: mm.files?.length ?? 0 }
    } catch {
      // Ignore parse errors
    }
  }
...
if (MODE === 'apply') {
  ...
  if (!existsSync(approvedPath)) {
    console.error('Error: No approved.json found. Run --approve first after reviewing the dry-run output.')
    process.exit(1)
  }

  const approvedRaw = JSON.parse(await readFile(approvedPath, 'utf8'))
  if (!approvedRaw.approved) {
    console.error('Error: approved.json has approved: false — set it to true before applying.')
    process.exit(1)
  }

  // Gate 2: hash verification
  if (!existsSync(manifestPath) || !existsSync(seedPath)) {
    console.error('Error: Manifest or seed files missing. Re-run --dry-run and --approve.')
    process.exit(1)
  }
  ...
  const latestManifestMs  = Math.max(manifestFileStat.mtimeMs, seedFileStat.mtimeMs)

  if (latestManifestMs > approvedFileStat.mtimeMs) {
    const newerFile = manifestFileStat.mtimeMs > seedFileStat.mtimeMs ? 'client-manifest.json' : 'seed-preview.sql'
    console.error(`Error: ${newerFile} is newer than approved.json — manifests changed after approval.`)
  ...
  if ('api_key_available' in approvedRaw) {
    ...
    if (keyNow !== approvedRaw.api_key_available) {
      ...
      console.error(`Error: GOOGLE_MAPS_API_KEY was ${was} during dry-run but is ${now} now.`)
      console.error('  Re-run --dry-run and --approve to regenerate with current API access.')
      process.exit(1)
    }
  }
  ...
  if (approvedRaw.media_dir && existsSync(approvedRaw.media_dir)) {
    ...
    if (currentDirStat.mtime.toISOString() !== approvedRaw.media_dir_mtime) {
      console.error(`Error: Image directory was modified after approval: ${approvedRaw.media_dir}`)
      console.error('  Re-run --dry-run and --approve if image changes are intentional.')
      process.exit(1)
    }
  }
  ...
  if (approvedRaw.invalidated) {
    console.error(`Error: Approval was invalidated — ${approvedRaw.invalidated_reason ?? 'overrides changed after approval'}`)
    console.error('  Re-run --approve after reviewing the regenerated manifests.')
    process.exit(1)
  }
  ...
  if (existsSync(mediaManifestPath)) {
    ...
    if (!media.files?.length && !ALLOW_STOCK) {
      ...
      console.error('  Add --allow-stock to proceed with stock/placeholder images, or provide an --images directory.')
      process.exit(1)
    }
    if (!media.files?.length && ALLOW_STOCK) {
      console.log('⚠ Proceeding with no client images (--allow-stock set). Update media before launch.')
    }
  }
  ...
  const siteId = `site-${SLUG}`
  const TRACKED = ['business_locations', 'reviews', 'location_qa', 'media_assets', 'experiences']
  const before = {}
  ...
  if (anyCountsAvailable) {
    ...
    for (const table of TRACKED) {
      ...
      const a = after[table]
      const delta = typeof b === 'number' && typeof a === 'number'
        ? (a - b > 0 ? `+${a - b}` : String(a - b))
        : '?'
      console.log(`  ${table.padEnd(22)} ${String(b).padStart(6)}   ${String(a).padStart(5)}   ${String(delta).padStart(5)}`)
    }
  }
...
if (!API_KEY) {
  console.log('  ⚠ GOOGLE_MAPS_API_KEY not set — Places API calls will be skipped')
}
...
if (mediaManifest.error) {
  ...
  if (!ALLOW_STOCK) {
    console.log('  ⚠ No client images found. Add --allow-stock to allow stock/placeholder images,')
    console.log('    or pass --images <dir> to provide client photos. --apply will refuse without one.')
  } else {
    console.log('  ⚠ --allow-stock set — proceeding without client images. Remember to update before launch.')
  }
} else {
  console.log(`  ✓ Found ${mediaManifest.total} images in ${mediaManifest.directory}`)
...
if (existsSync(overridesPath)) {
  try {
    ...
    const overrides = Object.fromEntries(Object.entries(raw).map(([k, v]) => [k, v.value]))
    const keys = Object.keys(overrides)
    if (keys.length > 0) {
      console.log(`\n→ Applying ${keys.length} override(s) from overrides.json...`)
      if (places[0]) {
        if (overrides.phone)               { places[0].phone = overrides.phone; places[0].international_phone = overrides.phone }
...
if (missing.length === 0) {
  ...
  for (const m of missing) {
    console.log(`  ⚠ ${m.location}: missing ${m.field}${m.issue ? ` (${m.issue})` : ''}`)
  }
}
...
const seedSql = generateSeedSql(places, mediaManifest.files ? mediaManifest : { files: [] })
...
function buildGeneratedCopyInventory(places, mediaManifest) {
  ...
  for (const place of places.filter(p => !p.error)) {
    ...
    inventory.push({
      ...
      note: place.description ? null : 'No description from Places API — fill before launch',
    })
  }
  ...
  for (const place of places.filter(p => !p.error && p.reviews?.length)) {
    for (const review of place.reviews) {
      inventory.push({
        table: 'reviews',
        field: 'content',
        value: review.text?.slice(0, 80) + (review.text?.length > 80 ? '…' : ''),
...
const generatedCount = generatedCopyInventory.filter(i => i.provenance === 'generated').length
const missingCount   = generatedCopyInventory.filter(i => i.provenance === 'missing').length
console.log(`  ✓ ${generatedCopyInventory.length} fields catalogued (${generatedCount} generated, ${missingCount} missing)`)
...
if (copyHits.length === 0) {
  ...
  for (const hit of copyHits) {
    console.log(`  ✗ Line ${hit.line}: pattern "${hit.pattern}" matched: ${hit.text}`)
  }
  copyReport = `FAIL — ${copyHits.length} forbidden copy hit(s)\n\n` +
...
const manifestForFile = JSON.parse(JSON.stringify(clientManifest, (k, v) => k === '_raw' ? undefined : v))
...
console.log(`\n┌─ Summary ──────────────────────────────────────────────────────`)
console.log(`│  Places resolved:  ${places.filter(p => !p.error).length} / ${places.length}`)
