...
if (!args.url) {
  console.error('Usage: node scripts/client-verify.mjs --url <site-url> --vertical <vertical> [--site-id <id>] [--slug <slug>]')
  process.exit(1)
}
...
const FORBIDDEN_COPY = {
  experience: [
    'Come dine with us', 'Reserve a table', 'From the kitchen',
    'one kitchen philosophy', 'Catering & events', "chef's table",
    'tasting menu', 'dine differently', 'Also part of Saya',
    'Ember & Slice', 'kikuzuki',
  ],
  restaurant: [],
  retail: [],
  wellness: [],
...
const CORE_ROUTES = ['/', '/locations', '/reviews', '/qa', '/posts', '/about', '/contact']
...
const routes   = REQUIRED_ROUTES_BY_VERTICAL[VERTICAL] ?? [...CORE_ROUTES, ...(VERTICAL_ROUTES[VERTICAL] ?? [])]
const pageHtml = {}
...
if (OUT_DIR) {
  ...
  if (existsSync(routeManifestPath)) {
    ...
    for (const route of [...(routeManifest.locations ?? []), ...(routeManifest.experiences ?? [])]) {
      if (pageHtml[route]) { pass(`Route manifest: ${route} (already verified)`); continue }
      const res = await get(route)
      if (res.ok) {
        pass(`Route manifest: GET ${route} → ${res.status}`)
        pageHtml[route] = await res.text()
      } else {
        const kind = routeManifest.locations?.includes(route) ? 'location' : 'experience'
        fail(`Route manifest: GET ${route} → ${res.status} — expected ${kind} slug missing`)
      }
    }
  }
}
...
if (SITE_ID && (VERTICAL === 'experience' || VERTICAL === 'restaurant')) {
  ...
  if (res.ok) {
    ...
    const items = VERTICAL === 'experience' ? (data.experiencesList ?? []) : (data.menu?.items ?? [])
    ...
    if (items.length === 0) {
      ...
      for (const item of items.slice(0, 5)) {
        ...
        if (r.ok) { pass(`GET ${route} → ${r.status}`); pageHtml[route] = await r.text() }
  ...
  if (data) {
    ...
    if (locs.length === 0) {
      ...
      for (const loc of locs) {
        ...
        if (pageHtml[route]) { pass(`GET ${route} → cached`); continue }
        const r = await get(route)
        if (r.ok) { pass(`GET ${route} → ${r.status}`); pageHtml[route] = await r.text() }
        else       { fail(`GET ${route} → ${r.status}`) }
      }
...
if (SITE_ID) {
  ...
  if (data) {
    ...
    const emails  = Object.values(data.config ?? {}).filter(v => typeof v === 'string' && v.includes('@'))
    ...
    if (allJson.includes('bamboo.chow@gmail.com') && VERTICAL !== 'experience') fail('Kikuzuki placeholder email found in production data')
    if (allJson.includes('Ember & Slice')) fail('Demo site data (Ember & Slice) found in production response')

    // Guard: static fallback phone must not be served
    if (allJson.includes('+66 81 270 2616') && !allJson.includes('bamboo')) {
      fail('Static Saya fallback phone number found — location phone is not client data')
    }
  }
}
...
if (OUT_DIR) {
  ...
  if (existsSync(mediaManifestPath)) {
    ...
    for (const file of (media.files ?? [])) {
      const localExists = file.source_file ? existsSync(file.source_file) : false
...
if (OUT_DIR) {
  ...
  if (existsSync(latestPath)) {
    try {
      ...
      await writeFile(previousPath, JSON.stringify(previousReport, null, 2), 'utf8')
    } catch {
  ...
  if (previousReport) {
    const prevByMsg  = new Map(previousReport.results.map(r => [r.message, r.status]))
    const diffLines  = [`Verify diff: ${previousReport.verified_at} → ${reportJson.verified_at}`, '─'.repeat(60)]
    let   regressions = 0
    let   fixes       = 0
    let   unchanged   = 0
    ...
    for (const [msg, status] of prevByMsg) {
      if (!newMsgs.has(msg)) diffLines.push(`  GONE  [${status.toUpperCase()}] ${msg}`)
    }

    diffLines.push('─'.repeat(60))
    diffLines.push(`  ${regressions} regression(s)  ${fixes} fix(es)  ${unchanged} unchanged`)
...
if (OUT_DIR && failures === 0) {
  ...
  if (data?.locations?.length) {
    ...
    for (const loc of data.locations) {
      ...
      if (loc.opening_hours) {
        ...
        try { hours = JSON.parse(loc.opening_hours)?.weekday_text?.join(', ') ?? hours } catch {
          // Ignore parse errors
        }
        handoffLines.push(`- Hours: ${typeof hours === 'string' ? hours.slice(0, 120) : JSON.stringify(hours).slice(0, 120)}`)
      }
      handoffLines.push('')
    }
  }
  ...
  const expRoutes = Object.keys(pageHtml).filter(r => r.startsWith('/experiences/'))
  if (expRoutes.length) {
  ...
  if (existsSync(mediaManifestPath)) {
    ...
    if (media.files?.length) {
      ...
      for (const f of media.files) {
        handoffLines.push(`- \`${f.public_url}\` — ${f.assigned_to} (${f.hash?.slice(0, 19) ?? 'no hash'})`)
      }
      handoffLines.push('')
    }
  }
  ...
  if (existsSync(missingPath)) {
    ...
    if (missing.length) {
      ...
      for (const m of missing) handoffLines.push(`- **${m.location}**: ${m.field}${m.issue ? ` (${m.issue})` : ''}`)
      handoffLines.push('')
    } else {
  ...
  handoffLines.push('## Verification Summary', '', `${passes} checks passed, ${failures} failed.`)

  const handoffPath = join(OUT_DIR, 'client-handoff.md')
  await writeFile(handoffPath, handoffLines.join('\n') + '\n', 'utf8')
  console.log(`    ${relative(process.cwd(), handoffPath)}  ← client handoff ready`)
}
