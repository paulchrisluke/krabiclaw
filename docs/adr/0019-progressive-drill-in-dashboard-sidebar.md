...
`layouts/dashboard.vue` renders exactly one `UDashboardSidebar` and one `UNavigationMenu` for every scope. A single `scope` computed (`'organization' | 'site' | 'location'`) is derived strictly from route params (`locationSlug` present → location; else `siteSlug` present → site; else organization) — never from `route.path` regexes or residual dashboard-context state, which would misclassify scope at an ancestor route once that state has been populated from a deeper page in the same session (this was a real bug during implementation: `useDashboardLocation().currentLocationSlug` falls back to a *preferred* location even outside a location route, and `site.value?.subdomain` persists after navigating back to the org root).
...
- Org-level settings (general/domains/members/billing) are intentionally distinct from the CMS registry's site-scoped `settings` manager ("Brand, navigation, footer & SEO") — conflating them was a bug found and fixed during implementation. Phase 1 of #316 gives each its own canonical route; until then, org settings surface via an explicit item in the organization-scope group.
