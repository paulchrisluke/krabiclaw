...
- Pull requests to `staging` that require deployed browser coverage build the
  exact Worker once, deploy preview plus `krabiclaw-e2e-1` and
  `krabiclaw-e2e-2`, and run a real two-lane smoke concurrently. Documentation-
  only changes skip Worker and lane deployment. This exercises lane-local host
