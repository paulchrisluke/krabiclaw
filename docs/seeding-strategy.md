...
3. **Replace its E2E coverage.** Either retire the assertions that depended on the seeded fixture, point them at read-only production canaries instead (same pattern as `prod-smoke`/canaries in AGENTS.md), or stand up a fresh synthetic tenant to cover the feature being tested (e.g. the second-location flow) without depending on a tenant that now has real client edits.
