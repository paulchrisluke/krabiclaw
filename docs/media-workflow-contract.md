...
- `open_video_upload` launches the single inline MCP Apps widget. It accepts video only and calls `window.openai.uploadFile`, `window.openai.getFileDownloadUrl`, then `window.openai.callTool('upload_user_media', ...)`. Images stay on direct ChatGPT attachments or native image generation. The dashboard media library remains a fallback only for chat clients that do not support inline widgets. Do not add semantic launcher variants such as `open_experience_media_upload` or restore `open_media_upload`; upload first, then place the asset with the appropriate assignment tool.
