...
Two separate things, not one: the **browser CSS viewport** used to load each page, and the **stored JPEG's pixel dimensions**. They are not the same number, and are not a fixed multiple of each other — the screenshot tool resamples full-page captures to its own internal limits, so stored dimensions vary with page height even within one capture session.

The 2026-09-01 evening batch (everything captured from the second re-login onward — second-location Settings sub-screens, Activity, org Settings, Support, Account, Onboarding, and both Menu-editor error captures) had its browser measured directly: `window.innerWidth` 500, `window.innerHeight` 701, `window.devicePixelRatio` 2. That describes the page as loaded, not the file written to disk. Earlier batches from prior sessions used different browser instances that can't be re-measured retroactively.

Actual **stored image dimensions** across every file in this half of the packet fall into four buckets: 500×701, 896×1357, 924×1291, 924×1295 — see `AUDIT.tsv` for the exact recorded dimensions of every individual file, cross-checked by `scripts/verify-cms-redesign-packet.py`.
...
These two states are **not simultaneous** — the working screenshots and the broken screenshots are roughly 75–140 minutes apart. An earlier draft of this note speculated that two same-day commits (`b8e5ef62`, `e2e91d3e`) caused and then reverted the break; that speculation was checked directly against both commits' diffs and found unsupported — neither touches `components/products/ProductEditor.vue`, `composables/useDashboardSite.ts`, or any other dashboard fetch/URL-construction code (`b8e5ef62` only touches `server/utils/mcp-*`; `e2e91d3e` only touches the public Saya storefront pages). **The cause is unknown, and so is the current state** — issue [#723](https://github.com/paulchrisluke/krabiclaw/issues/723) needs a direct re-test against current production, not a theory about what changed. The **public storefront menu was unaffected** throughout — `kikuzuki-thailand.com/menu` rendered correctly during the same window, so if this was a real regression it was confined to the dashboard editor.
...
| `dashboard/[orgSlug]/inbox` (org-level inbox list) | Not captured — guest names |
