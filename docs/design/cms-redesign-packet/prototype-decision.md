...
2. The Page Editor route owns the left Nuxt UI sidebar. Each large navigation card previews the content it opens, such as hero imagery and copy, CTA treatment, image thumbnails, rich text, or compact page metadata.
...
- Make every outline card communicate its content type before its label is read.
...
- Prefer progressive disclosure over dense controls on narrow viewports.
...
The approved comparison artifacts remain on the `prototype/cms-visual-language` branch. They use real local tenant data and never call a write endpoint. The production implementation should carry the selected interaction rules forward without importing prototype-only routes or variant controls.
