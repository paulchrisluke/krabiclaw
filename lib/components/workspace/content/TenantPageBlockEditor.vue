    ...
    <template v-else-if="block.type === 'gallery'">
      <div class="space-y-3">
        <div v-for="(media, index) in mediaForSlot('gallery')" :key="`${media.asset_id}-${index}`" class="flex items-center gap-3">
          ...
          <MediaPicker class="min-w-0 flex-1" :site-id="siteId" :model-value="media.asset_id" accept="image" @update:model-value="commitGalleryAsset(index, $event)" />
          <UButton icon="i-lucide-trash-2" color="error" variant="ghost" size="xs" aria-label="Remove gallery image" @click="commitGalleryAsset(index, null)" />
        </div>
        <div v-if="pendingNewGallerySlot" class="flex items-center gap-3">
          ...
          <MediaPicker class="min-w-0 flex-1" :site-id="siteId" :model-value="null" accept="image" @update:model-value="commitGalleryAsset('new', $event)" />
          <UButton icon="i-lucide-x" color="neutral" variant="ghost" size="xs" aria-label="Cancel adding image" @click="pendingNewGallerySlot = false" />
        </div>
        <UButton icon="i-lucide-plus" color="neutral" variant="soft" size="sm" :disabled="pendingNewGallerySlot" @click="pendingNewGallerySlot = true">Add image</UButton>
      </div>
    </template>
...
function commitGalleryAsset(index: number | 'new', assetId: string | null | undefined) {
  if (index === 'new') {
