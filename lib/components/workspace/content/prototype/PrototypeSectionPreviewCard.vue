<template>
  <button
    type="button"
    class="w-full overflow-hidden rounded-2xl border text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
    :class="selected
      ? 'border-primary bg-primary/5 ring-2 ring-primary/20'
      : 'border-default/70 bg-transparent hover:border-accented'"
    @click="emit('select')"
  >
    <span v-if="section.type === 'hero'" class="relative block min-h-44 overflow-hidden">
      ...
      <span class="relative flex min-h-44 flex-col justify-end p-5" :class="section.mediaUrl ? 'text-white' : 'text-highlighted'">
    ...
    <span v-else-if="isCallToAction" class="block p-5">
    ...
    <span v-else-if="isRichText" class="block p-5">
      <span class="text-[10px] font-bold uppercase tracking-[0.16em] text-muted">{{ section.label }}</span>
    ...
    <span v-else-if="isGrid" class="block p-5">
      <span class="text-[10px] font-bold uppercase tracking-[0.16em] text-muted">{{ section.label }}</span>
